...
type State = {
  ...
  rate: number;
  recordedFileURI:{  };
  recognizedImages: any[]; // Array to store recognized image data
  imageUris: []
};
...
export default class Home extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      rate: 1.0,
      recordedFileURI:{},
      recognizedImages: [], // Initialize recognizedImages as an empty array
      imageUris: []
    };
    this.recordingSettings = Audio.RecordingOptionsPresets.LOW_QUALITY;
  ...
  private processNetworkResponse = async (response: { recognized_images: any[]; }) => {
    if (response.recognized_images && response.recognized_images.length > 0) {
      const saveImagePromises = response.recognized_images.map((item, index) => {
        if (item.data && item.type === 'image') {
          const filename = `image_${index}.jpg`;
          return this.saveBase64Image(item.data, filename).then(() => {
      ...
      Promise.all(saveImagePromises).then((filePaths) => {
        const uris = filePaths.filter(path => path !== null);
        this.props.navigation.navigate('ImageDisplayScreen', { imageUris: uris });
      });
    }
  };
  
  private saveBase64Image = async (base64Data: string, filename: string) => {
    try {
  ...
  private onGenerateSigns = async () => {
    this.setState({
      isLoading: true,
    });
  
    const apiEndpoint = "http://192.168.0.213:105/speech-to-text";
    const fileUri = this.state.recordedFileURI.uri;
  
    // Prepare the file in FormData
    const formData = new FormData();
    formData.append("file", {
      uri: fileUri,
      type: "audio/3gp", // Set the correct MIME type
      name: "audio.3gp",
    });
    ...
    fetch(apiEndpoint, {
      method: "POST",
      body: formData, // No headers for multipart/form-data; fetch sets it
    })
    ...
    .then((data) => {
      ...
      if (data && data.recognized_images) {
        // Extract image URIs from the response
        const uris = data.recognized_images.map((item: { data: any; }) => item.data);
    ...
    .catch((error) => {
      console.error("Error in onGenerateSigns:", error);
      this.setState({
        isLoading: false,
      });
    });
  ...
  render() {
    ...
    const recognizedImagesList = this.state.recognizedImages.map((imageData, index) => (
      <Image key={index} source={{ uri: imageData.data }} style={styles.recognizedImage} />
    ));
    ...
    return (
      <View style={styles.container}>
        ...
        >
          ...
          >
            <Button
              title="Generate Signs"
              onPress={this.onGenerateSigns}
              disabled={!this.state.isPlaybackAllowed || this.state.isLoading}
              color="#B15A25"
            />  
        ...
        {/* Render recognized images */}
        <View style={styles.recognizedImagesContainer}>
          {recognizedImagesList}
        </View>
      </View>
    );
  }
